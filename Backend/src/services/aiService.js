...
//     6. IS_FIXED RULE: 
//       - Set "isFixed": true ONLY for hard appointments (Meetings, Doctor visits, Classes).
//       - Set "isFixed": false for solo tasks, leisure, or habits (Gym, Movie, Study, YouTube).
...
export const analyzeUserCommand = async (command, userContext) => {
  ...
  const systemPrompt = `
    ...
    1. INTENT: 
       ...
       - "REJECT_TASK": User declines a suggestion (e.g., "No", "Cancel", "I don't want to").
       - "RESCHEDULE", "QUERY", "FEEDBACK".
       - QUERY: Use this when the user asks "What's left today?", "Show my schedule", or "What's next?".
    2. TIME:
       - If user says "at 5", and it is currently 10 AM, assume 5 PM (17:00).
       - If user says "at 5", and it is currently 6 PM, assume 5 PM Tomorrow.
       - If no time is specified, assign a "suggestedTime" based on User's Work Hours.
    ...
    6. IS_FIXED RULE: 
      ...
      - Set "isFixed": false for solo tasks, leisure, or habits (Gym, Movie, Study, YouTube).
    7. QUERY LOGIC:
       - If intent is "QUERY", look at the "Upcoming Tasks" provided in the context.
       - Write a natural, human-like summary in the "responseMessage". 
       - If the task list is empty, tell them they are all caught up for the day!
